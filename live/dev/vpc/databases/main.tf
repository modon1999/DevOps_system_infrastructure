...
terraform {
  backend "s3" {
    ...
    key    = "dev/vpc/databases/terraform.tfstate"
    region = "eu-central-1"
...
data "terraform_remote_state" "password" {
  ...
  config = {
    ...
    key    = "dev/kms/terraform.tfstate"          // Object name in the bucket to GET Terraform state
...
module "mysql" {
  ...
  storage     = 10
  username_db = "admin"
  password_db = data.terraform_remote_state.password.outputs.password_db_dev
  subnets_db  = data.terraform_remote_state.network.outputs.dev_public_subnet_ids
  common_tags = {

...
terraform {
  backend "s3" {
    ...
    key    = "mgmt/services/bastion-host/stage/terraform.tfstate"
    region = "eu-central-1"
...
data "terraform_remote_state" "database" {
  ...
  config = {
    bucket = "modon-example-devops-project-state"    // Bucket from where to GET Terraform State
    key    = "stage/vpc/databases/terraform.tfstate" // Object name in the bucket to GET Terraform state
    region = "eu-central-1"                          // Region where bycket created
  }
}
...
module "basion_host" {
  ...
  vpc_id      = data.terraform_remote_state.network.outputs.staging_vpc_id
  subnet_id   = data.terraform_remote_state.network.outputs.staging_public_subnet_ids[0]
